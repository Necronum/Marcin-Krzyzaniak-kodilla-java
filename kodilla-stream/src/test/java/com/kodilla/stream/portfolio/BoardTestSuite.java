...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        OptionalDouble averageDays = project.getTaskLists().stream()
                ...
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .mapToDouble(d -> {if (LocalDate.now().getMonthValue() - d.getMonthValue() >=0) {
                    System.out.println("This check when dates are in the same year.");
                    return LocalDate.now().getDayOfYear() - d.getDayOfYear();
                } else {
                    LocalDate endOfYear = LocalDate.of(LocalDate.now().getYear(), 12, 31);
                    System.out.println("This check when dates are not in the same year.");
                    return d.getDayOfYear()+((endOfYear.getDayOfYear()+1) - LocalDate.now().getDayOfYear());
                }
                })
                .average();

        //Then
        assertEquals(10.0, averageDays.getAsDouble(), 0.01);
    }
